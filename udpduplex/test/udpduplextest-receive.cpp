...
int main(int argc, char *argv[])
{
    (void)argc;
    (void)argv;
    std::unique_ptr<UDPDuplex> udpDuplex{std::make_unique<UDPDuplex>("127.0.0.1", 8887, 8888)};
    ...
    do {
        if (readFuture.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready) {
            ...
            std::string stringToSend{str};
            std::cout << "    Rx << " << str << std::endl;
            std::cout << "    Tx >> " << stringToSend << std::endl;
            udpDuplex->writeString(stringToSend);
...
std::string readFromUdp(UDPDuplex *udpDuplex)
{
    do {
        if (udpDuplex->available()) {
            std::string str{udpDuplex->readStringUntil("}")};
            if (str.length() != 0) {
                return str;
            }
        }
